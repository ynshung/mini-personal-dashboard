...
- `LYRICS_FONT_SIZE` (default 17 px) controls current line size; context lines scale to `round(size * 0.72)`; `LYRICS_LATENCY_OFFSET_MS` (default 150 ms) compensates for network + render + decode delay; `LYRICS_ROMAJI` (default `false`) converts Japanese lyrics to romaji at render time — detected by hiragana/katakana presence (`[぀-ヿ]`), converted via `pykakasi`, English/symbol lines pass through unchanged
