...
- `LYRICS_JAPANESE_MODE` (default `off`) controls Japanese lyrics handling: `romaji` converts to Hepburn romanization via pykakasi at render time; `translate` translates the full song to English via Gemini (`gemini-3.5-flash`) once on first fetch, cached to `server/.translation_cache/{track_id}.json` — subsequent requests (including server restarts) skip the API call; `GEMINI_API_KEY` required for translate mode; detection uses hiragana/katakana presence (`[぀-ヿ]`)
