...
- `SPOTIFY`: polls `/v1/spotify/now-playing` every 5 s; response includes `current_line` (0-based lyric line index) and `next_line_at_ms` when `has_lyrics` is true — ESP stores these and enters lyrics mode; fetches `/v1/spotify/lyrics/frame?line=N` for rendering; after each frame, increments `currentLineIndex` and schedules next fetch from `X-Next-Line-At-Ms` header; every 5s poll refreshes `currentLineIndex` from server (corrects drift on seek/track change); when `has_lyrics` is false renders album art; progress bar always drawn locally
...
**Polling & rendering:**
- `/v1/spotify/now-playing` returns lightweight JSON: `track_id`, `is_playing`, `progress_ms`, `duration_ms`, `has_lyrics`; when `has_lyrics` is true, also includes `current_line` (0-based lyric line index, -1 if before first line) and `next_line_at_ms` (pre-adjusted by `LATENCY_OFFSET_MS`, -1 at end of song); also updates server-side playback cache used by the lyrics endpoint
...
- `/v1/spotify/lyrics/frame?line=N` returns 240×240 JPEG with blurred+dimmed album art background and 3-line lyrics overlay; `line` (0-based index) is required; response header `X-Next-Line-At-Ms` = `next_line_timestamp - LATENCY_OFFSET_MS` (pre-adjusted for pipeline latency), or `-1` at end of song; ESP increments `currentLineIndex` after each frame and schedules next fetch as `max(50, nextLineAt - localProgress)` ms from now
...
- **Eager Pre-rendering:** When lyrics are fetched or loaded, a background executor worker is started to pre-render the JPEGs for all lines of the track into a singular `server/.lyrics_cache/current_frames/` directory. An ownership file `current_frames/.track_id` identifies the owner. The directory is cleared and re-created atomically for every new track to keep the storage footprint minimal (~1 MB max).
- `GET /v1/spotify/lyrics/frame?line=N`: `line` (0-based index) is required; selects prev/curr/next lines by index; if pre-rendered frame exists and belongs to the correct track, reads and returns it directly (< 2 ms); otherwise, falls back to on-demand rendering (opens cached raw album art, applies Gaussian blur radius 10 + 60% black dim overlay, calls `composite_lyrics()`, and saves as JPEG with `optimize=True`) and triggers background pre-rendering; returns JPEG with `X-Next-Line-At-Ms = next_line_timestamp - LATENCY_OFFSET_MS` (pre-adjusted so ESP fires at the right moment), or `-1` at end of song
...
- `LYRICS_FONT_SIZE` (default 17 px) controls current line size; context lines scale to `round(size * 0.72)`; `LYRICS_LATENCY_OFFSET_MS` (default 150 ms) — used in `get_current_line()` (added to progress for lookahead) and in `X-Next-Line-At-Ms` (subtracted so ESP fires early enough to compensate for network latency); `LYRICS_ROMAJI` (default `false`) converts Japanese lyrics to romaji at render time — detected by hiragana/katakana presence (`[぀-ヿ]`), converted via `pykakasi`, English/symbol lines pass through unchanged
