...
- `GET /v1/spotify/lyrics/frame?line=N`: `line` (0-based index) is required; selects prev/curr/next lines by index; opens cached raw album art (no gradient), applies Gaussian blur (radius 10) + 60% black dim overlay; calls `composite_lyrics()`; returns JPEG with `X-Next-Line-At-Ms = next_line_timestamp - LATENCY_OFFSET_MS` (pre-adjusted so ESP fires at the right moment), or `-1` at end of song
